...
// 20 min per step (see lifecycle-copilot.test.cjs for rationale).
...
test('opencode lifecycle: new-project → plan-phase → execute-phase → verify-work', async (t) => {
  ...
  try {
    ...
    const s1 = await runStep(scratch.dir, 'Run /gsd-new-project --auto to initialize this repository as a GSD project. Use the README as the idea document. Choose all recommended defaults.');
    ...
    const s2 = await runStep(scratch.dir, 'Run /gsd-plan-phase 1 --auto to plan fixing the broken add() function in src/calc.js so tests/calc.test.js passes. Save the plan to disk.');
    ...
    const s4 = await runStep(scratch.dir, 'Run /gsd-verify-work 1 on the work just completed. Run the project tests as part of verification.');
