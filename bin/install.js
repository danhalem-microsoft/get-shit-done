...
const CODEX_AGENT_SANDBOX = {
  ...
  'gsd-critic-verify': 'read-only',
  'gsd-critic-discuss': 'read-only',
  'gsd-critic-strategy': 'read-only',
};
...
// Fork patch: code-search detection + template marker expansion
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Fork patch: detect code-search MCP server for template expansion
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        ...
        content = processAttribution(content, getCommitAttribution(runtime));
        // Fork patch: expand code-search template markers before runtime conversion
