...
> AI design contract template (legacy). The dedicated AI-integration-phase entry point and the eval-auditor agent have been retired in the Phase 1 cull; until a replacement is defined, this template is reference-only. Authors who fill it in by hand can still feed it to `gsd-planner` as input context.
...
> Domain context section. Grounds the evaluation strategy in domain expert knowledge. (The dedicated domain-researcher agent has been retired; gather this section by hand or via inline research from the orchestrator.)
...
> Framework reference section. Fetch from the framework's official docs (the dedicated AI-researcher agent has been retired; pull this content inline from the orchestrator using WebFetch + Read). Distilled for this specific use case.
...
> AI systems best-practices section. Cross-cutting patterns every developer building AI systems needs — independent of framework choice. (The dedicated AI-researcher agent has been retired; the orchestrator fills this section inline from its training plus targeted web research.)
