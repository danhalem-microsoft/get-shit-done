...
- `--copilot` → include GitHub Copilot CLI (preferred default)
...
- No flags and no model names in prompt → invoke both default reviewers:
  1. `copilot --model gpt-5.4` (via copilot CLI)
  2. `opencode run --model github-copilot/gemini-3.1-pro-preview` (via opencode CLI)
  If either CLI is missing, skip that reviewer and continue with the other.
  If model names ARE provided in the prompt, use copilot with those models instead of the defaults.

If no CLIs are available:
```
No external AI CLIs found. Install at least one:
- copilot: npm install -g @githubnext/github-copilot-cli (preferred — supports multiple models via --model)
...
Then run /gsd-peer-review again.
```
Exit.
...
Write to a temp file: `/tmp/gsd-peer-review-prompt-{phase}.md`
</step>
...
```bash
COPILOT_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get review.models.copilot --raw 2>/dev/null || true)
...
Model override from prompt: If the user specifies model names in the `/gsd-peer-review` arguments (e.g., `/gsd-peer-review 3 gpt-4.1 o4-mini`), those models override config for copilot. Parse non-flag, non-phase arguments as copilot model names. When multiple models are provided, invoke copilot once per model and label each review section accordingly (e.g., "Copilot Review (gpt-4.1)", "Copilot Review (o4-mini)").

For each selected CLI, invoke in sequence (not parallel — avoid rate limits):

**GitHub Copilot CLI (preferred default):**

Copilot supports `--model <model>` and `-p <text>` for non-interactive prompting. When multiple models are specified (from prompt args or config), invoke copilot once per model.

```bash
# Single model (from config or prompt)
if [ -n "$COPILOT_MODEL" ] && [ "$COPILOT_MODEL" != "null" ]; then
  copilot --model "$COPILOT_MODEL" -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-copilot-{phase}.md
else
  copilot -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-copilot-{phase}.md
fi
if [ ! -s /tmp/gsd-peer-review-copilot-{phase}.md ]; then
  echo "Copilot review failed or returned empty output." > /tmp/gsd-peer-review-copilot-{phase}.md
fi

# Multiple models from prompt args (e.g., "gpt-4.1 o4-mini"):
# For each MODEL in the list:
copilot --model "$MODEL" -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-copilot-${MODEL}-{phase}.md
```

**Gemini:**
```bash
if [ -n "$GEMINI_MODEL" ] && [ "$GEMINI_MODEL" != "null" ]; then
  gemini -m "$GEMINI_MODEL" -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-gemini-{phase}.md
else
  gemini -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-gemini-{phase}.md
fi
```
...
if [ -n "$CLAUDE_MODEL" ] && [ "$CLAUDE_MODEL" != "null" ]; then
  claude --model "$CLAUDE_MODEL" -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-claude-{phase}.md
else
  claude -p "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-claude-{phase}.md
fi
```
...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  codex exec --model "$CODEX_MODEL" --skip-git-repo-check "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-codex-{phase}.md
else
  codex exec --skip-git-repo-check "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-codex-{phase}.md
fi
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/gsd-peer-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
if [ -n "$OPENCODE_MODEL" ] && [ "$OPENCODE_MODEL" != "null" ]; then
  cat /tmp/gsd-peer-review-prompt-{phase}.md | opencode run --model "$OPENCODE_MODEL" - 2>/dev/null > /tmp/gsd-peer-review-opencode-{phase}.md
else
  cat /tmp/gsd-peer-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/gsd-peer-review-opencode-{phase}.md
fi
if [ ! -s /tmp/gsd-peer-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/gsd-peer-review-opencode-{phase}.md
fi
```

**Qwen Code:**
```bash
qwen "$(cat /tmp/gsd-peer-review-prompt-{phase}.md)" 2>/dev/null > /tmp/gsd-peer-review-qwen-{phase}.md
if [ ! -s /tmp/gsd-peer-review-qwen-{phase}.md ]; then
  echo "Qwen review failed or returned empty output." > /tmp/gsd-peer-review-qwen-{phase}.md
fi
```

**Cursor:**
```bash
cat /tmp/gsd-peer-review-prompt-{phase}.md | cursor agent -p --mode ask --trust 2>/dev/null > /tmp/gsd-peer-review-cursor-{phase}.md
if [ ! -s /tmp/gsd-peer-review-cursor-{phase}.md ]; then
  echo "Cursor review failed or returned empty output." > /tmp/gsd-peer-review-cursor-{phase}.md
fi
```
...
---
phase: {N}
reviewers: [copilot, gemini, claude, codex, coderabbit, opencode, qwen, cursor]
...
## Copilot Review ({model name})

{copilot review content — one section per model if multiple models were specified}
