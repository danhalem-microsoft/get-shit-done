...
grep -c "Pending" ${planning_root}/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files ${planning_root}/ROADMAP.md ${planning_root}/REQUIREMENTS.md
```
...
- `cat ${planning_root}/ROADMAP.md` — see updated roadmap
