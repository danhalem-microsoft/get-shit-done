...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    ...
    planning_root: planningRootPath,
    ...
    // File existence
    state_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'STATE.md')) : false),
    roadmap_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'ROADMAP.md')) : false),
    config_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'config.json')) : false),
    // File paths
    state_path: (planningRootPath ? `${planningRootPath}/STATE.md` : null),
    roadmap_path: (planningRootPath ? `${planningRootPath}/ROADMAP.md` : null),
  ...
  if (options.validate) {
    try {
      ...
      const statePath = path.join(cwd, planningRootPath, 'STATE.md');
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    ...
    state_path: (planningRootPath ? `${planningRootPath}/STATE.md` : null),
    roadmap_path: (planningRootPath ? `${planningRootPath}/ROADMAP.md` : null),
    requirements_path: (planningRootPath ? `${planningRootPath}/REQUIREMENTS.md` : null),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    ...
    has_codebase_map: (planningRootPath ? pathExistsInternal(cwd, `${planningRootPath}/codebase`) : false),
    planning_exists: pathExistsInternal(cwd, '.planning'),
    ...
    needs_codebase_map: (hasCode || hasPackageFile) && !(planningRootPath ? pathExistsInternal(cwd, `${planningRootPath}/codebase`) : false),
...
function cmdInitNewMilestone(cwd, raw) {
  ...
  const result = {
    ...
    phase_archive_path: (latestCompleted && planningRootPath) ? `${planningRootPath}/milestones/${latestCompleted.version}-phases` : null,
...
function cmdInitQuick(cwd, description, raw) {
  ...
  const result = {
    ...
    // Paths
    quick_dir: planningRootPath ? `${planningRootPath}/quick` : null,
    task_dir: (planningRootPath && slug) ? `${planningRootPath}/quick/${quickId}-${slug}` : null,
...
function cmdInitResume(cwd, raw) {
  ...
  try {
    interruptedAgentId = fs.readFileSync(path.join(cwd, planningRootPath, 'current-agent-id.txt'), 'utf-8').trim();
  } catch { /* intentionally empty */ }
...
function cmdInitTodos(cwd, area, raw) {
  ...
  const pendingDir = planningRootPath
    ? path.join(cwd, planningRootPath, 'todos', 'pending')
    : null;
  let count = 0;
  const todos = [];
    ...
    for (const file of files) {
      try {
        ...
        todos.push({
          ...
          area: todoArea,
          path: `${planningRootPath}/todos/pending/${file}`,
        });
      } catch { /* intentionally empty */ }
    }
  } catch { /* intentionally empty */ }
  ...
  const result = {
    ...
    // Paths
    pending_dir: (planningRootPath ? `${planningRootPath}/todos/pending` : null),
    completed_dir: (planningRootPath ? `${planningRootPath}/todos/completed` : null),

    // File existence
    planning_exists: pathExistsInternal(cwd, '.planning'),
    todos_dir_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'todos')) : false),
    pending_dir_exists: fs.existsSync(path.join(cwd, planningRootPath, 'todos', 'pending')),
  };
...
function cmdInitMilestoneOp(cwd, raw) {
  ...
  // Check archive
  const archiveDir = planningRootPath ? path.join(cwd, planningRootPath, 'archive') : null;
  ...
  const result = {
    ...
    roadmap_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'ROADMAP.md')) : false),
    state_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'STATE.md')) : false),
    archive_exists: fs.existsSync(path.join(cwd, planningRootPath, 'archive')),
    phases_dir_exists: (planningRootPath ? fs.existsSync(path.join(cwd, planningRootPath, 'phases')) : false),
  };
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const codebaseDir = planningRootPath ? path.join(cwd, planningRootPath, 'codebase') : null;
  let existingMaps = [];
  if (codebaseDir) {
    try {
  ...
  const result = {
    ...
    // Paths
    codebase_dir: planningRootPath ? `${planningRootPath}/codebase` : null,
    ...
    // File existence
    planning_exists: pathExistsInternal(cwd, '.planning'),
    codebase_dir_exists: (planningRootPath ? pathExistsInternal(cwd, `${planningRootPath}/codebase`) : false),
  };
...
function cmdInitProgress(cwd, raw) {
  ...
  try {
    const roadmapContent = extractCurrentMilestone(
      fs.readFileSync(path.join(cwd, planningRootPath, 'ROADMAP.md'), 'utf-8'), cwd
    );
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const phaseInfo = {
        number: phaseNumber,
        name: phaseName,
        directory: `${planningRootPath}/phases/${dir}`,
        status,
  ...
  let pausedAt = null;
  try {
    const state = fs.readFileSync(path.join(cwd, planningRootPath, 'STATE.md'), 'utf-8');
