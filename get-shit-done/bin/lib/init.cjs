...
const { loadConfig, resolveModelInternal, findPhaseInternal, getRoadmapPhaseInternal, pathExistsInternal, generateSlugInternal, getMilestoneInfo, getMilestonePhaseFilter, stripShippedMilestones, extractCurrentMilestone, normalizePhaseName, planningPaths, planningDir, planningRoot, toPosixPath, safeReadFile, output, error, checkAgentsInstalled, phaseTokenMatches, tryGetPlanningContext } = require('./core.cjs');
// Fork imports (FORK.md §Conflict Resolution — init.cjs row):
// Identity + active-context + project-listing helpers for the multi-user
// path-resolution layer. Used by withProjectRoot (extended below) to inject
// active_user/active_project/planning_root into every init output, and by the
// fork-only commands cmdInitSwitch / cmdInitProjectSetup / cmdArchiveProject /
// cmdRestoreProject / cmdInitTeamStatus (appended near end of file).
...
function withProjectRoot(cwd, result) {
  ...
  // Fork patch (FORK.md Phase 3 Pattern 1 — init-context injection):
  // Attach active_user/active_project/planning_root to every init output so
  // workflows can route to `.planning/users/<user>/<project>/` correctly and
  // surface identity in handoff blocks. tryGetPlanningContext returns null-valued
  // fields when no project is active (e.g. /gsd-new-project bootstrap), so we
  // always emit the keys — callers use presence (!== null) to gate behavior.
  try {
    const ctx = tryGetPlanningContext(cwd);
    result.active_user = ctx ? ctx.active_user : null;
    result.active_project = ctx ? ctx.active_project : null;
    result.planning_root = ctx ? ctx.planning_root : null;
  } catch {
    // Hard errors (CI/CD) propagate from tryGetPlanningContext. Anything else
    // (e.g. readdir races) → null identity, matches pre-project bootstrap shape.
    result.active_user = null;
    result.active_project = null;
    result.planning_root = null;
  }
  ...
  const projectMdPath = path.join(planningPaths(cwd).planning, 'PROJECT.md');
  try {
...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    ...
    // File existence
    state_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'STATE.md')),
    roadmap_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'ROADMAP.md')),
    config_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'config.json')),
    // File paths
    state_path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'STATE.md'))),
    roadmap_path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'ROADMAP.md'))),
    config_path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'config.json'))),
  };
  ...
  if (options.validate) {
    try {
      ...
      const statePath = path.join(planningPaths(cwd).planning, 'STATE.md');
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    ...
    // Environment
    planning_exists: fs.existsSync(planningPaths(cwd).planning),
    roadmap_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'ROADMAP.md')),

    // File paths
    state_path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'STATE.md'))),
    roadmap_path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'ROADMAP.md'))),
    requirements_path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'REQUIREMENTS.md'))),
...
function cmdInitTodos(cwd, area, raw) {
  ...
  const pendingDir = path.join(planningPaths(cwd).planning, 'todos', 'pending');
  let count = 0;
  const todos = [];
    ...
    for (const file of files) {
      try {
        ...
        todos.push({
          ...
          area: todoArea,
          path: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'todos', 'pending', file))),
        });
      } catch { /* intentionally empty */ }
    }
  } catch { /* intentionally empty */ }
  ...
  const result = {
    ...
    // Paths
    pending_dir: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'todos', 'pending'))),
    completed_dir: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'todos', 'completed'))),

    // File existence
    planning_exists: fs.existsSync(planningPaths(cwd).planning),
    todos_dir_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'todos')),
    pending_dir_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'todos', 'pending')),
  };
...
function cmdInitMilestoneOp(cwd, raw) {
  ...
  const result = {
    ...
    phases_dir_exists: fs.existsSync(path.join(planningPaths(cwd).planning, 'phases')),
  };
...
function cmdInitProgress(cwd, raw) {
  ...
  try {
    const roadmapContent = extractCurrentMilestone(
      fs.readFileSync(path.join(planningPaths(cwd).planning, 'ROADMAP.md'), 'utf-8'), cwd
    );
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const phaseInfo = {
        number: phaseNumber,
        name: phaseName,
        directory: toPosixPath(path.relative(cwd, path.join(planningPaths(cwd).planning, 'phases', dir))),
        status,
  ...
  let pausedAt = null;
  try {
    const state = fs.readFileSync(path.join(planningPaths(cwd).planning, 'STATE.md'), 'utf-8');
...
// --- Fork-only commands (FORK.md §Files Modified — init.cjs row) -------------
// Multi-user + identity + archive/restore commands added by the fork. These do
// not exist upstream; they sit alongside upstream commands using the shared
// helpers and exports untouched by the merge.
...
module.exports = {
  ...
  buildSkillManifest,
  cmdSkillManifest,
  // Fork-only (FORK.md §Files Modified — init.cjs row)
  cmdInitSwitch,
  cmdInitProjectSetup,
  cmdArchiveProject,
  cmdRestoreProject,
