...
const { escapeRegex, normalizePhaseName, comparePhaseNum, findPhaseInternal, getArchivedPhaseDirs, generateSlugInternal, getMilestonePhaseFilter, toPosixPath, output, error, getPlanningRoot } = require('./core.cjs');
...
function cmdFindPhase(cwd, phase, raw) {
  ...
  try {
    ...
    const result = {
      found: true,
      directory: toPosixPath(path.join(planningRoot, 'phases', match)),
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  const dirPath = path.join(cwd, planningRoot, 'phases', dirName);
  ...
  const result = {
    ...
    name: description,
    slug,
    directory: `${planningRoot}/phases/${dirName}`,
  };
