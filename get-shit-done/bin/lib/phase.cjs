...
function cmdPhasesList(cwd, options, raw) {
  const phasesDir = path.join(path.join(cwd, getPlanningRoot(cwd)), 'phases');
...
function cmdPhaseNextDecimal(cwd, basePhase, raw) {
  ...
  try {
    ...
    const roadmapPath = path.join(path.join(cwd, getPlanningRoot(cwd)), 'ROADMAP.md');
...
function cmdFindPhase(cwd, phase, raw) {
  ...
  try {
    ...
    const result = {
      found: true,
      directory: toPosixPath(path.relative(cwd, path.join(phasesDir, match))),
...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
    ...
    if (customId || config.phase_naming === 'custom') {
      ...
      const phasesOnDisk = path.join(path.join(cwd, getPlanningRoot(cwd)), 'phases');
    ...
    const dirPath = path.join(path.join(cwd, getPlanningRoot(cwd)), 'phases', _dirName);
  ...
  const result = {
    ...
    name: description,
    slug,
    directory: toPosixPath(path.join(path.relative(cwd, path.join(cwd, getPlanningRoot(cwd))), 'phases', dirName)),
...
function cmdPhaseAddBatch(cwd, descriptions, raw) {
  ...
  const results = withPlanningLock(cwd, () => {
    ...
    for (const description of descriptions) {
      ...
      const dirPath = path.join(path.join(cwd, getPlanningRoot(cwd)), 'phases', dirName);
      fs.mkdirSync(dirPath, { recursive: true });
      fs.writeFileSync(path.join(dirPath, '.gitkeep'), '');
...
function cmdPhaseRemove(cwd, targetPhase, options, raw) {
  ...
  const statePath = path.join(path.join(cwd, getPlanningRoot(cwd)), 'STATE.md');
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    withPlanningLock(cwd, () => {
      ...
      const reqPath = path.join(path.join(cwd, getPlanningRoot(cwd)), 'REQUIREMENTS.md');
  ...
  try {
    const configPath = path.join(path.join(cwd, getPlanningRoot(cwd)), 'config.json');
