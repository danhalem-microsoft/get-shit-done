...
function cmdPhaseNextDecimal(cwd, basePhase, raw) {
  ...
  try {
    ...
    const roadmapPath = path.join(planningPaths(cwd).planning, 'ROADMAP.md');
...
function cmdFindPhase(cwd, phase, raw) {
  ...
  try {
    ...
    const result = {
      found: true,
      directory: toPosixPath(path.join(path.relative(cwd, planningPaths(cwd).planning), 'phases', match)),
...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
    ...
    if (customId || config.phase_naming === 'custom') {
      ...
      const phasesOnDisk = path.join(planningPaths(cwd).planning, 'phases');
    ...
    const dirPath = path.join(planningPaths(cwd).planning, 'phases', _dirName);
  ...
  const result = {
    ...
    name: description,
    slug,
    directory: toPosixPath(path.join(path.relative(cwd, planningPaths(cwd).planning), 'phases', dirName)),
...
function cmdPhaseAddBatch(cwd, descriptions, raw) {
  ...
  const results = withPlanningLock(cwd, () => {
    ...
    for (const description of descriptions) {
      ...
      const dirPath = path.join(planningPaths(cwd).planning, 'phases', dirName);
      fs.mkdirSync(dirPath, { recursive: true });
      fs.writeFileSync(path.join(dirPath, '.gitkeep'), '');
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    withPlanningLock(cwd, () => {
      ...
      const reqPath = path.join(planningPaths(cwd).planning, 'REQUIREMENTS.md');
  ...
  try {
    const configPath = path.join(planningPaths(cwd).planning, 'config.json');
