...
// TODO(phase-7): Fork added 6 gsd-critic-* entries to the inline MODEL_PROFILES
// table (plan, code, scope, verify, discuss, strategy). Upstream extracted
// MODEL_PROFILES to model-profiles.cjs, so the fork's critic rows must be
// replayed in that file instead of here. This plan is scoped to core.cjs
// (files_modified), so the MODEL_PROFILES critic rows are deferred to Phase 7.
...
function planningDir(cwd, ws, project) {
  // Fork patch: delegate to tryGetPlanningContext for multi-user path resolution.
  // Upstream's planningDir grounds on `.planning/{GSD_PROJECT}/`, but the fork's
  // multi-user layout is `.planning/users/<user>/<project>/`. Without this patch,
  // every upstream caller of planningDir(cwd) — findPhaseInternal, loadConfig,
  // getMilestoneInfo, getRoadmapPhaseInternal, etc. — resolves to the flat root
  // and can't find phase directories, STATE.md, ROADMAP.md, or config.json.
  ...
  // Fork multi-user resolution: if an active project context exists, use its
  // planning_root instead of the flat `.planning/` path. This is the single
  // change that makes ALL upstream callers of planningDir work correctly in
  // the multi-user layout without per-site edits.
  let base;
  const ctx = tryGetPlanningContext(cwd);
  if (ctx && ctx.planning_root && !project) {
    // Multi-user structure active and no explicit GSD_PROJECT override
    base = path.join(cwd, ctx.planning_root);
  } else {
    base = path.join(cwd, '.planning');
...
function planningPaths(cwd, wsOverride) {
  // Fork path resolution (FORK.md Conflict Resolution — core.cjs row; Phase 3 Pattern 1):
  // Delegate to getPlanningRoot for the multi-user `.planning/users/<user>/<project>/`
  // structure. Fall back to flat `.planning/` when no active project exists (test
  // fixtures, pre-project init paths). Preserves workstream awareness via explicit
  // wsOverride or GSD_WORKSTREAM env var.
  const ctx = tryGetPlanningContext(cwd);
  let base;
  if (ctx && ctx.planning_root) {
    // Multi-user structure — re-invoke getPlanningRoot so the authoritative path
    // resolution (cache + hard errors) stays in the single chokepoint.
    base = path.join(cwd, getPlanningRoot(cwd));
  } else {
    // Flat `.planning/` fallback (FORK.md §Conflict Resolution row for core.cjs):
    // preserves backward compat with test fixtures and bootstrap flows before any
    // active project is registered.
    base = path.join(cwd, '.planning');
  }
  // Workstream awareness (FORK.md Phase 3 Pattern 1). Explicit wsOverride wins,
  // otherwise GSD_WORKSTREAM is consulted.
  const ws = wsOverride !== undefined ? wsOverride : process.env.GSD_WORKSTREAM;
  if (ws) {
    if (/[/\\]|\.\./.test(ws)) {
      throw new Error(`GSD_WORKSTREAM contains invalid path characters: ${ws}`);
    }
...
// --- Fork: Planning Root Resolution (FORK.md Conflict Resolution — core.cjs) ---
//
// Single chokepoint for multi-user path resolution. Added by the fork to route
// all planning I/O through `.planning/users/<user>/<project>/` instead of the
// upstream flat `.planning/` layout, while keeping a soft fallback to the flat
// layout for test fixtures and pre-project init paths.

let _planningRootCache = {};

/**
 * Authoritative resolver: returns the user-qualified planning root string
 * (e.g., `.planning/users/dan-halem/frontend`). Hard-errors on CI/CD or legacy
 * flat layouts. Used by all command sites that require an active project.
 */
...
/**
 * Soft wrapper used during init flows where a project may not yet exist.
 * Returns `{ active_user, active_project, planning_root }` with nulls on
 * graceful failure. CI/CD and legacy-flat-layout errors still propagate hard.
 */
...
// --- Fork: config-resolve command backing (ENV_KEY_MAP + cmdConfigResolve) ---
// Preserved from fork; called by gsd-tools.cjs `config-resolve <key>` for debugging.
...
function cmdConfigResolve(cwd, key, raw) {
  ...
  // Raw config file reads for per-layer value display. Upstream's loadConfig
  // merges the layers internally; here we peek at each layer independently so
  // the user can see which file contributed which value.
  const globalPath = path.join(cwd, '.planning', 'config.json');
  const ctx = tryGetPlanningContext(cwd);
  const projectPath = (ctx && ctx.planning_root)
    ? path.join(cwd, ctx.planning_root, 'config.json')
    : null;

  function readLayerValue(filePath) {
    if (!filePath) return null;
    try {
      const parsed = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
      if (parsed[key] !== undefined) return parsed[key];
      // Nested section lookups (parity with loadConfig's `get` helper)
      const nestedMap = {
  ...
  const layers = {
    default: CONFIG_DEFAULTS[key] !== undefined ? CONFIG_DEFAULTS[key] : null,
    global: readLayerValue(globalPath),
    project: readLayerValue(projectPath),
    env: envVal,
  };

  // TODO(phase-7): loadConfig in the merged upstream does not expose a
  // per-key `_sources` map (upstream simplified it out). Reporting the effective
  // source layer here requires either re-plumbing _sources through loadConfig or
  // reconstructing it from the layer reads above. Deferred to Phase 7 so this
  // plan stays scoped to core.cjs hand-merge only.
  output({
    key,
    value: config[key],
    source: null,
...
module.exports = {
  ...
  // Fork exports (FORK.md §"Conflict Resolution" — core.cjs)
  getPlanningRoot,
