...
const { output, error, getPlanningRoot, withPlanningLock, CONFIG_DEFAULTS, atomicWriteFileSync } = require('./core.cjs');
const {
  VALID_PROFILES,
...
function cmdConfigNewProject(cwd, choicesJson, raw) {
  const planningBase = path.join(cwd, getPlanningRoot(cwd));
