...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (!fs.existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run node gsd-tools.cjs verify --repair to regenerate', true);
      ...
      if (!diskPhases.has(ref) && !diskPhases.has(normalizedRef) && !diskPhases.has(String(parseInt(ref, 10)))) {
        ...
        if (diskPhases.size > 0) {
          addIssue(
            ...
            'Review STATE.md manually before changing it; node gsd-tools.cjs verify --repair will not overwrite an existing STATE.md for phase mismatches'
          );
        }
      }
    }
  ...
  if (!fs.existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run node gsd-tools.cjs verify --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${err.message}`, 'Run node gsd-tools.cjs verify --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && configParsed.workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run node gsd-tools.cjs verify --repair to add key', true);
        if (!repairs.includes('addNyquistKey')) repairs.push('addNyquistKey');
      }
      if (configParsed.workflow && configParsed.workflow.ai_integration_phase === undefined) {
        addIssue('warning', 'W016', 'config.json: workflow.ai_integration_phase absent (legacy key; the dedicated AI-integration-phase command was retired in the Phase 1 cull)', 'Run node gsd-tools.cjs verify --repair to add key', true);
      ...
      if (archivedVersions.length > 0) {
        ...
        if (missingFromRegistry.length > 0) {
          addIssue('warning', 'W018',
            ...
            'Run node gsd-tools.cjs verify --backfill to synthesize missing entries from archive snapshots',
            true);
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by node gsd-tools.cjs verify --repair\n`;
          ...
          case 'backfillMilestones': {
            ...
            for (const ver of missingFromRegistry) {
              try {
                ...
                const entry = `## ${ver}${milestoneName && milestoneName !== ver ? ` ${milestoneName}` : ''} (Backfilled: ${today})\n\n**Note:** Synthesized from archive snapshot by \`node gsd-tools.cjs verify --backfill\`. Original completion date unknown.\n\n---\n\n`;
