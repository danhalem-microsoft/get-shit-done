...
// Fork imports (FORK.md §Conflict Resolution — commands.cjs row):
// Identity + active-context helpers used by the fork-only cmdMigrate / cmdTeamStatus
// / cmdResearcher* functions appended below.
...
function cmdCommit(cwd, message, files, raw, amend, noVerify) {
  ...
  // Fork patch (FORK.md Pattern 4): auto-detect user/project for planning commit
  // attribution (TEAM-06). MUST run AFTER staging loop above so `git diff --cached`
  // sees the staged files; MUST run BEFORE commit below so the attribution prefix
  // lands in the actual commit message. Lazy-require core.cjs to avoid circular
  // init when this module is loaded from gsd-tools dispatcher.
  if (message && !amend) {
    try {
...
function cmdTodoComplete(cwd, filename, raw) {
  ...
  const pendingDir = path.join(planningPaths(cwd).planning, 'todos', 'pending');
  const completedDir = path.join(planningPaths(cwd).planning, 'todos', 'completed');
...
// --- Fork-only commands (FORK.md §Files Modified — commands.cjs row) ---------
// Researcher discovery (scan/load), team-status, and legacy-to-multi-user
// migration. These do not exist upstream; they sit alongside upstream commands.
    ...
    for (const file of files) {
      try {
        ...
        if (loaded) {
          results.push({
            name: loaded.name,
            output_file: loaded.output_file,
...
function loadResearcherType(filePath) {
  ...
  // Extract triggers list (YAML array format)
  const triggersMatch = fmBlock.match(/^triggers:\s*\n((?:\s+-\s+.+\n?)+)/m);
  if (triggersMatch) {
    fm.triggers = triggersMatch[1]
      .split('\n')
  ...
  return {
    name: fm.name || null,
    output_file: fm.output_file || null,
...
function cmdResearcherLoad(name, raw) {
  ...
  try {
    ...
    if (!loaded) {
      error('Could not parse researcher type: ' + name);
    }
    output(loaded, raw);
  } catch (e) {
    error('Researcher type not found: ' + name);
  }
}
...
module.exports = {
  ...
  cmdStats,
  cmdCheckCommit,
  // Fork-only (FORK.md §Files Modified — commands.cjs row)
  cmdResearcherScan,
  cmdResearcherLoad,
  scanResearcherTypes,
  loadResearcherType,
