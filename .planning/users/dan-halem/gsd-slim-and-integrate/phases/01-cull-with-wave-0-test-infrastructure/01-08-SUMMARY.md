...
### Tests with hardcoded references to deleted agents/commands (~50 subtests across ~15 files)

A wider sweep of `npm test` shows ~50 subtests fail across ~15 test files because they hardcode references to deleted agents (e.g., `gsd-debugger`, `gsd-doc-writer`, `gsd-eval-auditor`, `gsd-codebase-mapper`, `gsd-intel-updater`, `gsd-nyquist-auditor`, etc.) or deleted commands (e.g., `/gsd-autonomous`, `/gsd-debug`, `/gsd-stats`, etc.). Concrete files: `tests/agent-frontmatter.test.cjs`, `tests/agent-size-budget.test.cjs`, `tests/bug-1829-inherit-model-profile.test.cjs`, `tests/bug-2346-agent-read-loop-guards.test.cjs`, `tests/bug-2351-intel-kilo-layout.test.cjs`, `tests/bug-2396-makefile-test-priority.test.cjs`, `tests/bug-patterns-reference.test.cjs`, `tests/codex-config.test.cjs`, `tests/copilot-install.test.cjs`, `tests/core.test.cjs`, `tests/enh-2427-sycophancy-hardening.test.cjs`, `tests/model-profiles.test.cjs`, `tests/thinking-model-guidance.test.cjs`, plus tests/agent-skills-awareness.test.cjs (already fixed in commit `e981e2e8` as a Rule 1 deviation).

These are paired-test concerns the WARNING-fix #7 enumeration regex did not catch. Each test will need targeted updates: filter hardcoded agent/command name lists to only include surviving names, drop assertions tied to deleted artifacts, or replace example strings (e.g., copilot-install.test.cjs uses `/gsd-autonomous` as a fixture string for testing `gsd:` → `gsd-` conversion — Plan 08 already replaced 2 such strings in commit `358ae227`, but the file has more such uses that need similar treatment).

Triage strategy for the verifier (or the next planner): run `npm test 2>&1 | grep "^not ok"`, group failures by root cause (hardcoded deleted-agent string vs hardcoded deleted-command path vs deleted-command-paired-test), and fix each via Rule 1 deviations. None of these are blocking for the plan's stated success criteria (orphan-reference test GREEN, inventory tests GREEN, agent count = 22, command count = 43). They are technical debt left over from a paired-test enumeration regex that was too strict; cleanup is straightforward but mechanical.

### CLI module references to deleted agents (`bin/install.js`, `get-shit-done/bin/lib/*.cjs`)

Per RESEARCH.md §3.1, `bin/install.js` lines 30-42 hardcode agent name lists (e.g., `CODEX_AGENT_SANDBOX`, `claudeToCopilotTools`) that include deleted agents. `get-shit-done/bin/lib/model-profiles.cjs` lines 16-27 list per-agent model assignments for 8 deleted agents. `get-shit-done/bin/lib/intel.cjs` line 319, `docs.cjs` lines 15/251, and `init.cjs` line 922 reference deleted agent names by string.

The orphan-reference test correctly catches these (it scans `bin/install.js` and `get-shit-done/bin/lib/`), but the test was already GREEN by end of Plan 08 because Plan 06 already scrubbed prose-style references in CLI modules. The remaining hardcoded entries are name-list constants that don't match the orphan-reference scan patterns (which look for `@`-refs, slash-mentions, frontmatter mentions). They DO trip the broader test suite (e.g., `tests/codex-config.test.cjs` checks `CODEX_AGENT_SANDBOX` for the right roster).

This work is in scope of Plan 09 or a Phase 2 follow-up: prune deleted-agent name-list constants from CLI modules. The cull-no-orphan-references test will go RED again if those constants are left in and a follow-up adds slash-context patterns to the scan; for now the test stays GREEN because the constants are agent-name strings without leading `/gsd-` or `@`.
