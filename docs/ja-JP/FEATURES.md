...
- 既存のコードベースコンテキスト（_(retired)_ から取得）がある場合は読み込む
...
- REQ-PLAN-07: フロントエンドフェーズが検出され UI-SPEC.md が存在しない場合、システムはユーザーに _(retired)_ の実行を促さなければならない（UI セーフティゲート）
...
**コマンド:** _(retired)_、`/gsd-complete-milestone`、`/gsd-new-milestone [name]`
...
**ギャップクローズ:** _(retired)_ は監査で特定されたギャップを埋めるためのフェーズを作成します。
...
- REQ-DEBT-06: _(retired)_ はすべてのフェーズをスキャンし、項目をテスト可能性別にカテゴリ分けし、人的テストプランを生成しなければならない
...
**コマンド:** _(retired)_ <description>`、_(retired)_、_(retired)_ <idea>`
...
- REQ-BACKLOG-03: _(retired)_ は項目ごとにプロモート、維持、削除のアクションをサポートしなければならない
...
Claude が GSD ワークフローコンテキスト外でファイル編集を試行した際に検出する PreToolUse フック。直接編集の代わりに `/gsd-quick` や _(retired)_ の使用をアドバイスします。`hooks.workflow_guard`（デフォルト: false）で設定可能です。
...
**目的:** UI 重視のプロジェクトを自動検出し、_(retired)_ の推奨を表面化します。
...
- REQ-UI-DETECT-03: システムは UI 重視フェーズのネクストステップに _(retired)_ を提案しなければならない
- REQ-UI-DETECT-04: システムは _(retired)_ を必須にしてはならない

**プロセス:**
1. **検出** — プロジェクト説明と技術スタックの UI シグナル（キーワード、フレームワーク参照）をスキャン
2. **アノテーション** — ROADMAP.md の該当フェーズに `ui_hint` マーカーを追加
3. **表面化** — UI 重視フェーズのネクストステップに _(retired)_ の推奨を含める
...
**要件:**
- REQ-DOCS-01: システムはドキュメント生成のために _(retired agent)_ エージェントを起動しなければならない
- REQ-DOCS-02: システムは正確性チェックのために _(retired agent)_ エージェントを起動しなければならない
...
**プロセス:**
1. **生成** — _(retired agent)_ を起動して実装からドキュメントを作成・更新
2. **検証** — _(retired agent)_ を起動してコードベースに対するドキュメント正確性をチェック
...
**目的:** フェーズ依存関係を検出し、_(retired)_ 実行前に ROADMAP.md への `Depends on` エントリを提案します。
